    ...
    try:
        ...
        for project in all_projects:
            ...
            if user_member:
                ...
                # Build team members list safely, handling invalid user references
                team_members_data = []
                for member in project.team_members:
                    try:
                        # Access member.user to check if it exists (will raise DoesNotExist if invalid)
...
def api_create_project(request):
    ...
    try:
        ...
        # Automatically associate project with user's organization if user belongs to one
        ...
        # Save project first so it gets an ID (required before adding to organization.projects)
        project.save()

        # Now add project to organization's projects list if user belongs to an organization
        if user.organization:
            organization = user.organization
            if project not in organization.projects:
    ...
    try:
        ...
        for item in project_history:
            combined_history.append({
                ...
                'local_path': getattr(item, 'local_path', None) or '',
                'prompt': item.prompt,
        ...
        for item in individual_history:
            combined_history.append({
                ...
                'local_path': getattr(item, 'generated_image_path', None) or '',
                'prompt': item.prompt,
                'original_prompt': item.original_prompt,
...
def api_prompt_master_list(request):
    ...
    try:
        ...
        for prompt in prompts:
            try:
                # Safely handle created_by reference (may point to deleted user)
                # Access _data directly to avoid automatic dereferencing
                created_by_id = None
                try:
                    created_by_ref = prompt._data.get('created_by')
                    if created_by_ref:
                        # If it's a DBRef, extract the ID directly
                        if isinstance(created_by_ref, DBRef):
                            created_by_id = str(created_by_ref.id)
                        # If it's already dereferenced (Document instance)
                        elif hasattr(created_by_ref, 'id'):
                            created_by_id = str(created_by_ref.id)
                        # If it's just an ObjectId
                        else:
                            created_by_id = str(created_by_ref)
                except (DoesNotExist, AttributeError, Exception) as e:
                    # User was deleted, reference is orphaned
                    created_by_id = None

                # Safely handle updated_by reference (may point to deleted user)
                updated_by_id = None
                try:
                    updated_by_ref = prompt._data.get('updated_by')
                    if updated_by_ref:
                        # If it's a DBRef, extract the ID directly
                        if isinstance(updated_by_ref, DBRef):
                            updated_by_id = str(updated_by_ref.id)
                        # If it's already dereferenced (Document instance)
                        elif hasattr(updated_by_ref, 'id'):
                            updated_by_id = str(updated_by_ref.id)
                        # If it's just an ObjectId
                        else:
                            updated_by_id = str(updated_by_ref)
                except (DoesNotExist, AttributeError, Exception) as e:
                    # User was deleted, reference is orphaned
                    updated_by_id = None
                ...
                # Skip this prompt if there's any error processing it
...
def api_prompt_master_create(request):
    ...
    try:
        ...
        # Safely handle created_by and updated_by (should be fine for new prompts, but adding safety)
        # Access _data directly to avoid automatic dereferencing
        created_by_id = None
        try:
            created_by_ref = prompt._data.get('created_by')
...
def api_prompt_master_update(request, prompt_id):
    ...
    try:
        ...
        # Safely handle updated_by reference (should be current user, but adding safety)
        updated_by_id = None
        try:
            updated_by_ref = prompt._data.get('updated_by')
            if updated_by_ref:
