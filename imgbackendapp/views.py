    ...
    if form.is_valid():
        ...
        try:
            ...
            if has_genai:
                ...
                # -----------------------------
                if bg_b64:
                    bg_img = Image.open(BytesIO(base64.b64decode(bg_b64))).convert("RGB")
                    bg_img = bg_img.resize(ornament_img.size)
                else:
                    bg_img = Image.new("RGB", ornament_img.size, bg_color or (255, 255, 255))

                bg_img.paste(ornament_img, (0, 0), ornament_img.convert("RGBA"))
                buf = BytesIO()
            ...
            local_generated_path = os.path.join(gen_dir, f"generated_{ornament.name}")
