...
def upload_ornament(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            task = enqueue_task_to_user_queue(
                generate_white_background_task,
                user_id,
...
def change_background(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            return JsonResponse({
                "success": True,
                "message": "Background change task queued successfully",
                "task_id": task.id,
                "status": "PENDING"
            })
...
def generate_model_with_ornament(request):
    ...
    try:
        ...
        return JsonResponse({
            "status": "success",
            "message": "Model with ornament generation task queued successfully",
            "task_id": task.id,
            "status": "PENDING"
        }, status=200)
...
def generate_real_model_with_ornament(request):
    ...
    try:
        ...
        return JsonResponse({
            "status": "success",
            "message": "Real model with ornament generation task queued successfully",
            "task_id": task.id,
            "status": "PENDING"
        }, status=200)
...
def generate_campaign_shot_advanced(request):
    ...
    try:
        ...
        return JsonResponse({
            "status": "success",
            "message": "Campaign shot generation task queued successfully",
            "task_id": task.id,
            "status": "PENDING"
        }, status=200)
...
def regenerate_image(request):
    ...
    try:
        ...
        return JsonResponse({
            "success": True,
            "message": "Image regeneration task queued successfully",
            "task_id": task.id,
            "status": "PENDING",
            "image_id": image_id
        }, status=200)
...
def get_task_status(request):
    ...
    try:
        ...
        from imgbackend.celery import app as celery_app
        ...
        if task_result.state == 'PENDING':
            ...
            response_data["error"] = str(task_result.info) if task_result.info else "Unknown error"
        else:
