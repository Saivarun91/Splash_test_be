...
def create_organization(request):
    ...
    try:
        ...
        if not owner:
            ...
            owner = User(
                email=owner_email,
                password=make_password("temp_password_123"),  # Temporary password - should be changed on first login
                username=username,
                full_name=name.split()[0] if name else username,  # Use first word of org name or username
                role=Role.USER
            )
            try:
                owner.save()
        ...
        if initial_credits > 0:
            add_credits(organization, request.user, initial_credits, reason="Initial credit allocation")
...
def add_user_to_organization(request):
    ...
    try:
        ...
        organization_role = data.get('organization_role', 'member')  # owner, editor, chief_editor, member
        ...
        valid_roles = ['owner', 'editor', 'chief_editor', 'admin', 'member']
        ...
        org_role = OrgRole(
            user=user,
            organization=organization,
            role=OrgRoleType[organization_role.upper()] if hasattr(OrgRoleType, organization_role.upper()) else OrgRoleType.MEMBER,
            created_by=request.user,
            updated_by=request.user
        )
        org_role.save()
...
def remove_organization_credits(request, organization_id):
    ...
    try:
        ...
        result = deduct_credits(organization, request.user, amount, reason=reason)
